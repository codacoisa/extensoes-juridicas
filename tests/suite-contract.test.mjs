...
test('Customizações reverte integralmente recursos visuais', () => {
  ...
  assert.match(source, /const customHeaderIframeClearanceCss = settings\.customHeaderEnabled && isIframeDocument[\s\S]{0,300}?\.divCorpo > \.area:first-child,[\s\S]{0,160}?margin-top: 6px !important;/, 'o cabeçalho personalizado não compensa a margem negativa do título interno');
  ...
  assert.match(source, /table :is\(th, td\)\[data-pjc-column-kind="quantity"\] \{\s*text-align: center !important;/, 'o cabeçalho Qtde não acompanha o alinhamento dos valores');
